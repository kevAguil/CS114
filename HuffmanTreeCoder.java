...
public class HuffmanTreeCoder {
    ...
    public void buildTree(Map<Character,Double> freqMap) {
        ...
        while(!queue.isEmpty()) {
        	...
        	HuffmanTreeNode rootNode = new HuffmanTreeNode(node1.weight + node2.weight, node1, node2);
        	queue.add(rootNode);
        	}
        }
        // have a while loop (queue not empty) : 
        // in the loop, do a queue.remove() and store it in variable. 
        // remaining node becomes root
        /*
         * complete code here for lab assignment
         */
        encodeChars(root,"");
    ...
    private int decodeBitString(HuffmanTreeNode node,String bitString, int bitStrIndex, StringBuilder decodedString) {
    	if(node.isLeaf()) {
    		decodedString.append(node.ch);
    		return bitStrIndex;
    	}
    	else {
    		if(bitString.charAt(bitStrIndex) == '0') {
    			return decodeBitString(node.left, bitString, bitStrIndex+1, decodedString); 
    		}
    		if(bitString.charAt(bitStrIndex) == '1') {
    			return decodeBitString(node.right, bitString, bitStrIndex+1, decodedString); 
    		}    		
    	}
    	return bitStrIndex;
    }
    ...
    public static void main(String [] args) throws Exception {
      ...
      System.out.println("Decoded message = " +val);
        freqMap = new HashMap<>();
        int [] freqArr = new int [] {64, 13, 22, 32, 103, 21, 15, 47, 57, 1, 5, 32,20, 57, 63, 15, 1, 48, 51, 80, 23, 8, 18, 1, 16, 1, 186};
